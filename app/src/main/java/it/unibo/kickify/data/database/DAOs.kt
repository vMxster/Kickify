...
@Dao
interface CartDao {
    @Query("SELECT * FROM CARRELLO WHERE Email = :email")
    ...
    @Query("SELECT * FROM comprendere WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :color AND Taglia = :size LIMIT 1")
    ...
    @Query("UPDATE comprendere SET Quantita = :quantity WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :color AND Taglia = :size")
    suspend fun updateCartItemQuantity(cartId: Int, productId: Int, color: String, size: Double, quantity: Int): Int

    @Query("DELETE FROM comprendere WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :color AND Taglia = :size")
    suspend fun removeFromCart(cartId: Int, productId: Int, color: String, size: Double): Int

    @Query("SELECT c.*, p.Nome, p.Prezzo, p.Genere FROM comprendere c JOIN PRODOTTO p ON c.ID_Prodotto = p.ID_Prodotto WHERE c.ID_Carrello = :cartId")
    suspend fun getCartItems(cartId: Int): List<CartWithProductInfo>

    @Query("UPDATE comprendere SET Colore = :newColor WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :oldColor AND Taglia = :size")
    suspend fun updateCartItemColor(cartId: Int, productId: Int, oldColor: String, newColor: String, size: Double): Int

    @Query("UPDATE comprendere SET Taglia = :newSize WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :color AND Taglia = :oldSize")
    suspend fun updateCartItemSize(cartId: Int, productId: Int, color: String, oldSize: Double, newSize: Double): Int

    @Query("UPDATE comprendere SET Colore = :newColor, Taglia = :newSize WHERE ID_Carrello = :cartId AND ID_Prodotto = :productId AND Colore = :oldColor AND Taglia = :oldSize")
    ...
    @Transaction
    @Query("UPDATE CARRELLO SET Valore_Totale = (SELECT SUM(c.Quantita * p.Prezzo) FROM comprendere c JOIN PRODOTTO p ON c.ID_Prodotto = p.ID_Prodotto WHERE c.ID_Carrello = :cartId) WHERE ID_Carrello = :cartId")
    suspend fun updateCartTotal(cartId: Int): Int

    @Query("DELETE FROM comprendere WHERE ID_Carrello = :cartId")
...
interface OrderDao {
    ...
    @Transaction
    @Query("SELECT * FROM ORDINE o WHERE o.Email = :email ORDER BY o.Data_Ordine DESC")
    ...
    suspend fun getOrderTracking(orderId: Int): OrderDetailedTracking {
        ...
        if (rawData.isEmpty()) return OrderDetailedTracking(
            order_info = null,
            tracking_states = emptyList(),
            products = emptyList()
        )

        val first = rawData.first()
        val orderInfo = OrderBasicInfo(
            shipping_type = first.shipping_type,
        ...
        val trackingStates = states.map { state ->
            ...
            TrackingState(
                ...
                estimated_arrival = matchingStatus?.estimated_arrival ?: "",
                actual_arrival = matchingStatus?.actual_arrival
            )
        }

        val uniqueProducts = mutableMapOf<String, OrderProductDetail>()
        rawData.forEach { row ->
            val productKey = "${row.product_id}_${row.size}_${row.color}"
...
interface WishlistDao {
    @Query("SELECT p.* FROM PRODOTTO p JOIN aggiungere a ON p.ID_Prodotto = a.ID_Prodotto WHERE a.Email = :email ORDER BY p.Nome")
    suspend fun getWishlistItems(email: String): List<Product>

    @Insert
    suspend fun addToWishlist(wishlistProduct: WishlistProduct): Long

    @Query("DELETE FROM aggiungere WHERE Email = :email AND ID_Prodotto = :productId")
    suspend fun removeFromWishlist(email: String, productId: Int): Int

    @Query("DELETE FROM aggiungere WHERE Email = :email")
    suspend fun clearWishlist(email: String): Int

    @Query("SELECT COUNT(*) FROM aggiungere WHERE Email = :email")
    suspend fun getWishlistCount(email: String): Int

    @Query("SELECT COUNT(*) FROM aggiungere WHERE Email = :email AND ID_Prodotto = :productId")
...
interface ReviewDao {
    ...
    @Query("SELECT r.*, u.Nome, u.Cognome FROM RECENSIONE r JOIN UTENTE u ON r.Email = u.Email WHERE r.ID_Prodotto = :productId AND r.Data_Recensione > :lastAccess ORDER BY r.Data_Recensione DESC")
    suspend fun getProductReviews(productId: Int, lastAccess: String): List<ReviewWithUserInfo>

    @Query("DELETE FROM RECENSIONE WHERE Email = :email AND ID_Prodotto = :productId")
    suspend fun deleteReview(email: String, productId: Int): Int

    @Query("SELECT AVG(Voto) FROM RECENSIONE WHERE ID_Prodotto = :productId")
    suspend fun getProductRating(productId: Int): Double?

    @Query("SELECT COUNT(*) FROM ORDINE o JOIN PRODOTTO_ORDINE po ON o.ID_Ordine = po.ID_Ordine WHERE o.Email = :email AND po.ID_Prodotto = :productId")
...
interface NotificationDao {
    ...
    @Query("SELECT n.*, m.Corpo as MessaggioCompleto FROM NOTIFICA n LEFT JOIN MESSAGGIO m ON n.Timestamp_Invio = m.Timestamp_Invio AND n.TipoNotifica = 'Admin Message' WHERE n.Email = :email ORDER BY n.Timestamp_Invio DESC")
    suspend fun getUserNotifications(email: String): List<NotificationWithMessage>

    @Query("UPDATE NOTIFICA SET Tipo = 'Read' WHERE Email = :email")
    suspend fun markAllNotificationsAsRead(email: String): Int

    @Query("UPDATE NOTIFICA SET Tipo = 'Read' WHERE Email = :email AND ID_Notifica IN (:notificationIds)")
    suspend fun markNotificationsAsRead(email: String, notificationIds: List<Int>): Int

    @Query("SELECT COUNT(*) FROM NOTIFICA WHERE Email = :email AND Tipo = 'Unread'")
